...
import { dadosComprovante } from "@/app/(app)/comprovante-actions";
import {
  ComprovanteQuitacao,
  ComprovanteVenda,
  EspelhoCliente,
} from "@/components/receipt/fiado-receipt";
import { Button } from "@/components/ui/button";
import type { PedidoComprovante } from "@/lib/comprovante";
import { urlDoComprovanteComFormato } from "@/lib/comprovante";

import printStyles from "./print-page.module.css";

export type FormatoEmissao = "pdf" | "imagem";

type EmConstrucao = Extract<
  Awaited<ReturnType<typeof dadosComprovante>>,
  { ok: true }
>;

type ArquivoPronto = { file: File; titulo: string };

/** Desktop = mouse/trackpad (mesma heurística do auto-print do shell). */
export function isDesktop(): boolean {
  return window.matchMedia("(hover: hover) and (pointer: fine)").matches;
}

/**
 * Emissão direta de comprovantes no celular (F5b): busca os dados via server
 * action, renderiza o papel FORA da tela, gera o arquivo (PNG hi-def via
 * html-to-image; PDF embutindo o PNG via jsPDF) e abre o compartilhamento
 * nativo do aparelho — sem abrir a rota /comprovante/*. No desktop, `emitir`
 * apenas abre a rota de preview em nova aba, como sempre.
 *
...
export function useEmissorComprovante() {
  ...
  async function emitir(pedido: PedidoComprovante, formato: FormatoEmissao) {
    // Desktop: preview em nova aba (auto-print no PDF), fluxo de sempre.
    if (isDesktop()) {
      window.open(urlDoComprovanteComFormato(pedido, formato), "_blank");
      return;
    }
    ...
    try {
      const res = await dadosComprovante(pedido);
      if (!res.ok) {
      ...
      await compartilhar(file, `${res.titulo} — FiadoApp`);
    } catch {
      toast.error("Não foi possível gerar o documento. Tente de novo.");
    } finally {
      toast.dismiss(toastId);
  ...
  const node = (
    <>
      {emConstrucao ? (
        ...
        >
          <div ref={paperRef} className={printStyles.paper}>
            {emConstrucao.comprovante.tipo === "venda" ? (
              <ComprovanteVenda
                data={emConstrucao.comprovante.data}
                marca={emConstrucao.marca}
              />
            ) : emConstrucao.comprovante.tipo === "quitacao" ? (
              <ComprovanteQuitacao
                data={emConstrucao.comprovante.data}
                marca={emConstrucao.marca}
              />
