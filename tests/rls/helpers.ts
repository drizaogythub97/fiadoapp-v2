...
export async function createTestUser(label: string): Promise<TestUser> {
  ...
  const email = `rls-fiado-${label}-${stamp}@example.com`;
  ...
  const { data: created, error: createError } =
    await admin.auth.admin.createUser({
      ...
      password,
      email_confirm: true,
      // Banco compartilhado com o Gaveta: o trigger handle_new_user cria a
      // linha em profiles a partir destes metadados.
      user_metadata: {
...
/** Cria um cliente fiado para o usuário e retorna o id. */
export async function createCliente(
  accessToken: string,
  userId: string,
  nome: string,
): Promise<string> {
  const app = userClient(accessToken);
  const { data, error } = await app
    ...
    .single();
  if (error || !data) {
    throw new Error(`Falha ao criar cliente ${nome}: ${error?.message}`);
  }
