...
export async function updateSession(
  ...
  // requisição — cada página, cada navegação interna. A conferência COM
  // ESTADO (sessão apagada, usuário removido) continua acontecendo uma vez
  // por página no layout autenticado, via `obterUsuario()`; e o banco valida
  // a assinatura em toda consulta pela RLS. O que este arquivo decide é só
  ...
  //
  // Vale principalmente para o app instalado no celular: cada ida e volta
  // custa a latência inteira da rede.
  if (pathname === "/") {
  ...
  // Em /login e /signup — e SÓ aqui — a conferência é COM ESTADO.
  //
  ...
  // redirecionamentos" em vez de na tela de entrar. Isso ACONTECEU DE
  // VERDADE no Gaveta (PR #50) e os 110 testes e2e de lá NÃO pegaram — só a
  // prova dedicada de sessão revogada pegou.
  //
  // Como quem já está logado raramente pede /login, a viagem ao Auth aqui
  // não pesa; e quando a sessão de fato morreu, os cookies velhos são
  // limpos para a tela de entrar nascer limpa.
